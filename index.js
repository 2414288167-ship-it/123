...
function loadSettings() {
  const saved = window.extensions.getStorage("auto-ai-message");
  if (saved) {
    extensionSettings.autoAiMessage = { ...extensionSettings.autoAiMessage, ...saved };
  }
}

// 保存配置（官方规范：使用debounce避免频繁保存）
function saveSettings() {
  window.extensions.setStorage("auto-ai-message", extensionSettings.autoAiMessage);
}
